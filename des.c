...
static void ComputeRoundKey(bool roundKey[56], bool key[64]);
...
/*
 *  EncryptDES: Encrypt a block using DES. 
...
/*
 *  DecryptDES: Decrypt a block using DES.
...
static void ComputeRoundKey(bool roundKey[56], bool key[64]) {
  ...
  for (i = 63; i >= 0; i--) {
    if (i % 8 != 7) {
      roundKey[table_DES_PC1[counter++]] = key[i];
    }
  }
}
...
static void ComputeIP(bool L[32], bool R[32], bool inBlk[64]) {
   ...
   */
  for (i = 63; i >= 0; i--)
    output[table_DES_IP[i]] = inBlk[63-i];
...
static void ComputeFP(bool outBlk[64], bool L[32], bool R[32]) {
   ...
   */
  for (i = 63; i >= 0; i--)
    outBlk[table_DES_FP[i]] = input[63-i];
}
